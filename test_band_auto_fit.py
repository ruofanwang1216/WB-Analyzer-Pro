...
class BandAutoFitTests(unittest.TestCase):
    ...
    def test_signal_peaks_align_lane_crops_without_rescaling(self) -> None:
        ...
        self.assertEqual(len(result.lane_crop_boxes), 2)
        self.assertEqual(result.lane_crop_boxes[0].w, result.lane_crop_boxes[1].w)
        self.assertEqual(result.lane_crop_boxes[0].h, result.lane_crop_boxes[1].h)
        first_peak_in_crop = 46.0 - result.lane_crop_boxes[0].y
        second_peak_in_crop = 56.0 - result.lane_crop_boxes[1].y
        self.assertAlmostEqual(first_peak_in_crop, second_peak_in_crop)
